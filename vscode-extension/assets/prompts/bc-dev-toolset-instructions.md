...
When working with Business Central AL projects, use the **BC Dev Toolset** MCP server tools (`bc_dev_toolset_*`) for every supported operation, including compile, build, and validation. The toolset carries workspace context that a direct compiler invocation may miss, including package-cache settings, workspace dependencies, and `al.assemblyProbingPaths` for .NET assemblies.
...
| **Workspace** | `bc_dev_toolset_build_all_apps` — Compile workspace apps in dependency order with their configured package caches and assembly probing paths |
...
4. **Compile/build/validate requests** — If `bc_dev_toolset_build_all_apps` is exposed, call it for ordinary AL compilation, build, or validation requests. Do not invoke an AL compiler directly while this MCP tool is available. The MCP operation is responsible for discovering the active workspace and forwarding package-cache and assembly-probing settings.
5. **No failure bypass** — If a matching MCP operation fails, times out, lacks the terminal bridge, or returns incomplete context, report that result and the required corrective action. Do not retry the same operation through a direct compiler, Docker, BcContainerHelper, or PowerShell command unless the user explicitly requests that fallback.
   The AL test operation's compiled report includes diagnostics for its failed stage; do not rerun its nested build or tests merely to retrieve output.
   If `bc_dev_toolset_al_runner_test` reports an AL Runner tool failure, state that AL tests were not executed and suggest `bc_dev_toolset_invoke_tests` as the container-based fallback. Do not invoke the fallback automatically unless the user asks. Keep genuine AL compilation or test failures distinct from runner-tool failures.
6. **CLI only when unavailable or explicitly requested** — A direct AL CLI build is allowed only when `bc_dev_toolset_build_all_apps` is not exposed, or when the user explicitly asks for CLI execution. First call `bc_dev_toolset_get_workspace` when available and carry its package-cache and `al.assemblyProbingPaths` settings into the compiler command. Use the platform-specific ALTool bundled with the active Microsoft AL Language extension; never use the unsigned global `al` .NET tool. If required assembly context is absent, stop instead of guessing paths.
7. **Terminal bridge** — PowerShell-backed MCP operations require the BC Dev Toolset VS Code extension terminal bridge. If the MCP tool reports the bridge is unavailable, tell the user to start or reload the VS Code extension host.
...
9. **PowerShell fallback** — Manual PowerShell/terminal commands are appropriate only for work not covered by a `bc_dev_toolset_*` MCP tool, for reading local files, and for normal codebase maintenance.

...
4. **Compile/build/validate requests** — If `bc_dev_toolset_build_all_apps` is exposed, call it for ordinary AL compilation, build, or validation requests. Do not invoke `al.exe`, `altool.exe`, `alc.exe`, `al compile`, or a manually reconstructed PowerShell build while this MCP tool is available. The MCP operation is responsible for discovering the active workspace and forwarding package-cache and assembly-probing settings.
...
6. **CLI only when unavailable or explicitly requested** — A direct AL CLI build is allowed only when `bc_dev_toolset_build_all_apps` is not exposed, or when the user explicitly asks for CLI execution. First call `bc_dev_toolset_get_workspace` when available and carry its package-cache and `al.assemblyProbingPaths` settings into the compiler command. If required assembly context is absent, stop instead of guessing paths.

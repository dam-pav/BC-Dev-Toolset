...
The AL test MCP operation returns a compiled report instead of replaying every successful prerequisite step. It includes build, container-preparation, and test stage status; successful prerequisite stages are reduced to their status. A failed build reports the first 20 recognized AL compiler errors while omitting compiler warnings and informational entries; if compiler errors cannot be recognized safely, the complete captured build diagnostics are returned. Other failed stages include their captured diagnostics. Successful test runs return totals, while failed runs return up to 20 individual test failure details. The full operation remains visible in the terminal.
...
- *Run AL test tool tests* builds all workspace apps in dependency order before preparing the test container, so a failed build stops the operation before backup restore or deployment. Business Central discovers every codeunit in each workspace extension whose `SubType` is `Test`; test suite registration from an `OnInstall` procedure is not required. Discovered codeunits are partitioned into codeunit-isolated tests (runner `130450`) and explicitly selected non-isolated tests (runner `130451`). Both groups contribute to one MCP report, and an assertion failure in one group does not prevent the other from running. Empty groups are neutral; missing or malformed results and unavailable runner/filter capabilities fail the operation.
- Selective isolation is not restricted to any Business Central platform version. Compatibility is determined by the available test-tool and BcContainerHelper capabilities. Because a test-tool page can replace its extension selection when a range is applied, the Toolset discovers by extension first and sends only that app's discovered IDs in each pass's explicit range, including default isolated runs. After a non-isolated pass (also on failure), a no-test reset selects runner `130450` and range `0`, clearing the persistent DEFAULT suite selection without executing tests. BcContainerHelper must support extension discovery, `testCodeunitRange`, and `testRunnerCodeunitId`; no weaker-isolation fallback is used.
